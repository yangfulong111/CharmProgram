myApp.controller('AECutManualOrderImportController', ['AECutManualOrderService', '$scope', '$http',
	function(AECutManualOrderService, $scope, $http) {
		 ...
		 * 点击import,页面显示上传组件。
		 */
		$scope.importAeOrder = function() {
			$scope.importVisible = false;
			$scope.importSelect = true;
		};

		/**
		 * 选中文件后，点击submit后，上传文件
		 */
		$scope.uploadFile = function(files) {
			console.log("#############uploadFile.start######controller#################");

			AECutManualOrderService.uploadFile($scope);// 导出查询条件范围内容的数据
			console.log("#############uploadFile.end######controller#################");
		...
		$scope.gridOptions = {
			...
			// ----------- 选中 ----------------------
			enableFooterTotalSelected : true, // 是否显示选中的总数，默认为true,
			// 如果显示，showGridFooter 必须为true
			...
			modifierKeysToMultiSelect : false,// 默认false,为true时只能
			// 按ctrl或shift键进行多选, multiSelect
			// 必须为true;
		...
		var getPage = function(curPage, pageSize) {
			...
			// 或者像下面这种写法
			// $scope.myData = mydefalutData.slice(firstRow, firstRow +
			// pageSize);
		};
