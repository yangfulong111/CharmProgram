myApp.controller('AECutManualOrderController', [
	'AECutManualOrderService',
	'$scope',
	'$http',
	function(AECutManualOrderService, $scope, $http) {
		...
		$http.get('https://cdn.rawgit.com/angular-ui/ui-grid.info/gh-pages/data/500_complex.json').success(
			function(data) {
				// for (var i = 0; i < data.length; i++) {
				// data[i].state = data[i].address.state;
				// data[i].balance =
				// Number(data[i].balance.slice(1).replace(/,/, ''));
				// }
				data[0].$$treeLevel = 0;
				data[1].$$treeLevel = 1;
				data[10].$$treeLevel = 1;
